...
void initClocks(){
  ...
  for(int i = 0; i < clockX.length; i++){
    ...
    clockY[i] = SOIL_SIZE*floor(i*4+floor(random(4)));
...
void draw() {
	...
	switch (gameState) {
		...
		for(int i = 0; i < cabbageX.length; i++){
			...
			if(playerHealth<PLAYER_MAX_HEALTH && isHit(cabbageX[i], cabbageY[i] ,cabbage.width ,cabbage.height,playerX,playerY,80,80)){
      playerHealth++;
      cabbageX[i] = -1000;
       }
		}
    ...
    for(int i = 0; i < clockX.length; i++){
      ...
      image(clock, clockX[i], clockY[i]);
        ...
        if(isHit(clockX[i], clockY[i] ,clock.width ,clock.height,playerX,playerY,80,80)){
      addTime(15);
      clockX[i]=-1000;
       }
        ...
        if(isHit(soldierX[i], soldierY[i] ,soldier.width ,soldier.height,playerX,playerY,80,80)){
...
String convertFramesToTimeString(int frames){// Requirement #4
 ...
 color getTimeTextColor(int frames){				// Requirement #5
  ...
  if( frames > 7200) textColor = #00ffff;
  if( frames < 7200 && frames > 3600 ) textColor = #ffffff;
  if( 1800 < frames && frames < 3600) textColor = #ffcc00;
  if( 1800 > frames && frames > 600 ) textColor = #ff6600;
  if( frames < 600 ) textColor = #ff0000;
  return textColor ;
	}
